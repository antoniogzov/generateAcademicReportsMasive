async function getBangHighMixedSR(data) {
  for (let i_group = 0; i_group < data.groups_heb.length; i_group++) {
    ...
    for (
      ...
      for (
        let assignments = 0;
        assignments <
        array_all_periods_heb_qualifications_cond[2].period_qualifications
          .length;
        assignments++
      ) {
        var asignatura = [];
   ...
   */
     ...
     doc.text(230, 15, group_name); */
      ...
      doc.text(250, 190, "Fecha de emisión: " + output);
      ...
      doc.text(165, 189, "Firma del Tutor de Grupo");
